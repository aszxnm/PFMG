...
class CustomDataset(Dataset):
    def __init__(self, args, loader_type, augmentation=None, kwargs=None, build_cache=True):
        ...
        self.pose_world = args.pose_world
        self.pose_vel = args.pose_vel
...
#                     audio_each_file = audio_each_file[::sr//16000]
                    ...
                    continue
                if self.audio_norm:
                    # print(self.std_audio, (audio_each_file.max() - self.mean_audio) / self.std_audio, (audio_each_file.min() - self.mean_audio) / self.std_audio)
                    audio_each_file = (audio_each_file - self.mean_audio)
                    audio_each_file = audio_each_file / self.std_audio
            ...
            if self.pose_pae is not None:
                logger.info(f"# ---- Building cache for pae {id_pose} and Pose {id_pose} ---- #")
                pae_file = pose_file.replace(self.pose_rep, self.pose_pae).replace("bvh", "txt")
                try:
                    pae_data = np.loadtxt(pae_file, dtype=float, delimiter=' ')
                    pae_each_file = np.array(pae_data)
                except:
                    logger.warning(f"# ---- file not found for pae {id_pose}, skip all files with the same id ---- #")
                    continue                

            if self.pose_world is not None:
                logger.info(f"# ---- Building cache for world {id_pose} and Pose {id_pose} ---- #")
                world_file = pose_file.replace(self.pose_rep, self.pose_world).replace(".bvh", "_worldpos.csv")
                try:
                    world_data = np.loadtxt(world_file, dtype=float, delimiter=',', skiprows=1)
                    world_each_file = np.array(world_data)
                    vel_each_file = np.zeros((world_each_file.shape[0], world_each_file.shape[1]))
                    vel_each_file[1:] = world_each_file[1:] - world_each_file[:-1]
                except:
                    logger.warning(f"# ---- file not found for world {id_pose}, skip all files with the same id ---- #")
                    continue   
            ...
            filtered_result = self._sample_from_clip(
                dst_lmdb_env,
                audio_each_file, pose_each_file, facial_each_file, pae_each_file, world_each_file, vel_each_file, 
                word_each_file, vid_each_file, emo_each_file, sem_each_file,
    ...
    def _sample_from_clip(
        self, dst_lmdb_env, audio_each_file, pose_each_file, facial_each_file, pae_each_file, world_each_file, vel_each_file, 
        word_each_file, vid_each_file, emo_each_file, sem_each_file,
        disable_filtering, clean_first_seconds, clean_final_seconds, is_test,
        ):
        """
        ...
        for ratio in self.multi_length_training:
            ...
            for i in range(num_subdivision): # cut into around 2s chip, (self npose)
                ...
                sample_pae = pae_each_file[start_idx:fin_idx] if pae_each_file != [] else np.array([-1]) 
                sample_world = world_each_file[start_idx:fin_idx] if world_each_file != [] else np.array([-1])
                sample_vel = vel_each_file[start_idx:fin_idx] if vel_each_file != [] else np.array([-1]) 
                ...
                if sample_pose.any() != None:
                    ...
                    if is_correct_motion or disable_filtering:
                        ...
                        sample_pae_list.append(sample_pae)
                        sample_world_list.append(sample_world)
                        sample_vel_list.append(sample_vel)
            ...
            if len(sample_pose_list) > 0:
                with dst_lmdb_env.begin(write=True) as txn:
                    for pose, audio, facial, pae, world, vel, word, vid, emo, sem in zip(sample_pose_list,
                                                        sample_audio_list,
                                                        sample_facial_list,
                                                        sample_pae_list,
                                                        sample_world_list,
                        ...
                        v = [normalized_pose, audio, facial, pae, world, vel, word, emo, sem, vid]
    ...
    def __getitem__(self, idx):
        with self.lmdb_env.begin(write=False) as txn:
            ...
            if idx >= self.n_samples:
                print(idx, self.n_samples)

            sample = txn.get(key)
            if sample == None:
                logger.warning(f'Should Stop {self.loader_type} dataset iteration')
                raise StopIteration

            sample = pyarrow.deserialize(sample)
            tar_pose, in_audio, in_facial, in_pae, in_world, in_vel, in_word, emo, sem, vid = sample
            ...
            in_pae = torch.from_numpy(in_pae).float()  
            in_world = torch.from_numpy(in_world).float()  
            in_vel = torch.from_numpy(in_vel).float()  
            ...
            return {"pose":tar_pose, "audio":in_audio, "facial":in_facial, "pae":in_pae, "world": in_world, "velocity": in_vel, "word":in_word, "id":vid, "emo":emo, "sem":sem}
