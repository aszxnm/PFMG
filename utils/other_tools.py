...
def record_trial(args, csv_path, best_scores, best_epoch):
    score_name = []
    score_value = []
    score_epoch = []
    list4print = []
    ...
    for k, v in best_scores.items():
        score_name.append(k)
        score_value.append(v)
        score_epoch.append(best_epoch[k])
    
    if not os.path.exists(csv_path):
        with open(csv_path, "a+") as f:
            csv_writer = csv.writer(f)
            csv_writer.writerow([*score_name, *score_name, *name4print])
            
    with open(csv_path, "a+") as f:
        csv_writer = csv.writer(f)
        csv_writer.writerow([*score_value, *score_epoch, *list4print])
