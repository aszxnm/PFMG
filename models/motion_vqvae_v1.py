...
def _loss_fn(x_target, x_pred):
    recon_loss = t.abs(x_pred - x_target)
    recon_loss = t.mean(recon_loss, dim=(1, 2)) 
    return t.sum(recon_loss) 

class VQVAE(nn.Module):
    def __init__(self, args, input_dim=114):
        super().__init__()

        input_shape = (34, input_dim)
        levels = 1
        downs_t = [3,]
        strides_t = [2,]
        emb_width = 512
        ...
        block_kwargs = dict(width=512, depth=3, m_conv=1.0, \
    ...
    def forward(self, x):
        ...
        # loss = commit_loss * self.commit + self.reg * regularization + recons_loss +  self.vel * velocity_loss + self.acc * acceleration_loss
        ...
        return x_out, commit_loss, metrics
