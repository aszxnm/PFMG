...
class PoseGenerator(nn.Module):
    ...
    def __init__(self, args):
        ...
        # gating_indices = torch.tensor([(476 + i) for i in range(10)])
        # main_indices = torch.tensor([(0 + i) for i in range(476)])
        # dropout = 0.3
        # gating_hidden = 64
        # main_hidden = 1024
        # experts = 5
        ...
        # gating_indices = torch.tensor([(501 + i) for i in range(10)])
        # main_indices = torch.tensor([(0 + i) for i in range(501)])
        # dropout = 0.3
        # gating_hidden = 64
        # main_hidden = 1024
        # experts = 5
        ...
        self.out_hands = nn.Sequential(
            nn.Linear(self.hidden_size, self.hidden_size//2),
            nn.LeakyReLU(True),
            nn.Linear(self.hidden_size//2, 114)
        )
...
class PfMG_VQ(PoseGenerator):
    def __init__(self, args):
        ...
        self.decoder_body = nn.ModuleList()
        # self.final_out_body = nn.ModuleList()

        norm = 'ln'
        self.decoder_body.append(nn.Sequential(
            ConvNormRelu(self.hidden_size, self.hidden_size, norm=norm),
            ConvNormRelu(self.hidden_size, self.hidden_size, norm=norm),
            ConvNormRelu(self.hidden_size, self.hidden_size, norm=norm),
        ))
        # self.final_out_body.append(nn.Conv1d(self.hidden_size, self.hidden_size, 1, 1))

        self.decoder_hand = nn.ModuleList()
        # self.final_out_hand = nn.ModuleList()

        self.decoder_hand.append(nn.Sequential(
            ConvNormRelu(self.hidden_size, self.hidden_size, norm=norm),
            ConvNormRelu(self.hidden_size, self.hidden_size, norm=norm),
            ConvNormRelu(self.hidden_size, self.hidden_size, norm=norm),
        ))
        # self.final_out_hand.append(nn.Conv1d(self.hidden_size, self.hidden_size, 1, 1))

    def forward(self, pre_seq, in_audio=None, in_facial=None, in_pae=None, in_text=None, in_id=None, in_emo=None):
        if self.do_flatten_parameters:
            self.LSTM.flatten_parameters()
        ...
        in_facial, hidden_states_cont1, hidden_states_emo11_256 = self.audio2face(in_audio, in_text, in_id, in_emo)
        in_data = torch.cat((in_data, text_feat_seq), 2) if in_data is not None else text_feat_seq

        audio_feat_seq = torch.cat([hidden_states_cont1, hidden_states_emo11_256], dim=2)
        ...
        hidden_feature = self.decoder_body[0](hidden_feature).transpose(1, 2)
        # hidden_feature = self.final_out_body[0](hidden_feature).transpose(1, 2)
        decoder_outputs_iperiod = self.out(hidden_feature)
        decoder_outputs_period, _ = self.body_GNN(in_data)
        decoder_outputs = decoder_outputs_period + decoder_outputs_iperiod

        in_data = torch.cat((decoder_outputs, in_data), dim=2)
        hidden_feature, _ = self.LSTM_hands(in_data[:, :, :-40])
        hidden_feature = hidden_feature[:, :, :self.hidden_size] + hidden_feature[:, :, self.hidden_size:]
        hidden_feature = hidden_feature.transpose(1, 2)
        hidden_feature = self.decoder_hand[0](hidden_feature).transpose(1, 2)
        # hidden_feature = self.final_out_hand[0](hidden_feature).transpose(1, 2)
        decoder_outputs_hands_iperiod = self.out_hands(hidden_feature)
        decoder_outputs_hands_period, _ = self.hand_GNN(in_data)
        decoder_outputs_hands = decoder_outputs_hands_iperiod + decoder_outputs_hands_period
