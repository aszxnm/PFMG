...
class PoseGenerator(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.out = nn.Sequential(
            ...
            nn.LeakyReLU(True),
            nn.Linear(self.hidden_size//2, 141)
        )

        self.out_face = nn.Sequential(
            nn.Linear(self.hidden_size, self.hidden_size//2),
            nn.LeakyReLU(True),
            nn.Linear(self.hidden_size//2, 51)
        )
    ...
    def forward(self, pre_seq, in_audio=None, in_facial=None, in_text=None, in_id=None, in_emo=None):
        ...
        if in_audio is not None:
            ...
            if pre_seq is not None:
                if (audio_feat_seq.shape[1] != pre_seq.shape[1]):
                    min_gap = pre_seq.shape[1] - audio_feat_seq.shape[1]
        ...
        # print(speaker_feat_seq.shape, emo_feat_seq.shape, text_feat_seq.shape, audio_feat_seq.shape,face_feat_seq.shape, pre_seq.shape)
        in_data = torch.cat((pre_seq, in_data), dim=2)
        output, _ = self.LSTM(in_data)
        output = output[:, :, :self.hidden_size] + output[:, :, self.hidden_size:] 
        decoder_face = self.out_face(output)
        output = self.out(output.reshape(-1, output.shape[2]))
        decoder_outputs_final = output.reshape(in_data.shape[0], in_data.shape[1], -1)
        
        return decoder_outputs_final, decoder_face
