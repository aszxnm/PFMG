...
class Decoder(nn.Module):
    def __init__(self, out_dim, embedding_dim, num_hiddens, num_residual_layers, num_residual_hiddens, ae=False):
        ...
        self.project = nn.Conv1d(self._num_hiddens // 4, out_dim, 1, 1, 1)
...
class VQVAE(nn.Module):
    ...
    def __init__(self, args, in_dim=114, embedding_dim=64, num_embeddings=2048,
        ...
        self.vq_layer = VectorQuantizerEMA(embedding_dim, num_embeddings, commitment_cost, decay, training=False)
    ...
    def forward(self, gt_poses, id=None, pre_state=None):
        ...
        if not self.training:
            e, latent = self.vq_layer(z)
            x_recon, cur_state = self.decoder(e, pre_state.transpose(1, 2) if pre_state is not None else None)
            # print('gt_poses:', gt_poses, 'gt_recon:', x_recon.transpose(1, 2), 'latent:', latent)
            return e, x_recon.transpose(1, 2)
    ...
    def encode(self, gt_poses, id=None):
        ...
        # print(gt_poses.shape, z.shape, e.shape, latents.shape)
        return e, latents
    ...
    def decode(self, b, w, e=None, latents=None, pre_state=None):
        if e is not None:
            ...
            e = e.view(b, -1, w).contiguous()
            e = e.permute(0, 2, 1).contiguous()
            x, _ = self.decoder(e, pre_state.transpose(1, 2) if pre_state is not None else None)
            # x = t.stack(x)
