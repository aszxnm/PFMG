...
class PoseGenerator(nn.Module):
    ...
    def __init__(self, args):
        ...
        model_path = args.root_path + '/data/beat_cache/beat_4english_15_141/weights/face.bin'
        load_checkpoints(self.audio2face, model_path, 'face')
        for param in self.audio2face.parameters():
        ...
        self.batch=256
        self.width=64
        self.b_vqvae = VQVAE(args, 27)
        model_path = args.root_path + '/data/beat_cache/beat_4english_15_141/weights/b_vqvae.bin'
        # model_ckpt = torch.load(model_path, map_location=torch.device('cuda'))
        load_checkpoints(self.b_vqvae, model_path, 'body')
        for param in self.b_vqvae.parameters():
            param.requires_grad=False
        ...
        self.h_vqvae = VQVAE(args, 114)
        model_path = args.root_path + '/data/beat_cache/beat_4english_15_141/weights/h_vqvae.bin'
        load_checkpoints(self.h_vqvae, model_path, 'hand')
        for param in self.h_vqvae.parameters():
            param.requires_grad=False

        self.LSTM_hands = nn.LSTM(self.in_size+27, hidden_size=self.hidden_size, num_layers=args.n_layer, batch_first=True,
                          bidirectional=True, dropout=args.dropout_prob)

        self.body_linear = nn.Linear(34, 64)
        self.hand_linear = nn.Linear(34, 64)
...
class PfMG_VQ(PoseGenerator):
    ...
    def forward(self, pre_seq, in_audio=None, in_facial=None, in_pae=None, in_text=None, in_id=None, in_emo=None, in_pose=None):
        ...
        in_facial, text_feat_seq, audio_feat_seq = self.audio2face(in_audio, in_text, in_id, in_emo)
        ...
        if in_pae is not None:
            in_data = torch.cat((in_data, in_pae[:, :, 10:]), dim=2)
            in_data = torch.cat((in_data, in_pae[:, :, :10]), dim=2)
        ...
        if in_pose is not None:
            body_pose = torch.cat((in_pose[:, :, :18], in_pose[:, :, 75:84]), dim=2)
            hand_pose = torch.cat((in_pose[:, :, 18:75], in_pose[:, :, 84:]), dim=2)
            _, latent_body = self.b_vqvae.encode(body_pose)
            _, latent_hand = self.h_vqvae.encode(hand_pose)

        # print(self.args.batch_size, latent_body.shape)
        in_data = torch.cat((pre_seq, in_data), dim=2)
        out_data = in_data

        hidden_feature, _ = self.LSTM(in_data[:, :, :-40])
        hidden_feature = hidden_feature[:, :, :self.hidden_size] + hidden_feature[:, :, self.hidden_size:]
        hidden_feature = hidden_feature.transpose(1, 2)
        hidden_feature = self.body_linear(hidden_feature)
        hidden_feature = hidden_feature.transpose(1, 2).contiguous()
        body_hidden_feature = hidden_feature.view(self.args.batch_size, 8, -1)
        hidden_feature = torch.argmax(body_hidden_feature, dim=2)
        # print(hidden_feature.shape, hidden_feature)

        decoder_outputs_iperiod = self.b_vqvae.decode(self.args.batch_size, self.width, latents=latent_body)
        decoder_outputs_body = self.b_vqvae.decode(self.args.batch_size, self.width, latents=hidden_feature)
        decoder_outputs = decoder_outputs_iperiod
        # decoder_outputs = decoder_outputs_iperiod
        # print('body_pose:', body_pose[0], 'predicted_body:', decoder_outputs[0], 'latent_body:', latent_body[0])

        in_data = torch.cat((decoder_outputs, in_data), dim=2)
        hidden_feature, _ = self.LSTM_hands(in_data[:, :, :-40])
        hidden_feature = hidden_feature[:, :, :self.hidden_size] + hidden_feature[:, :, self.hidden_size:]
        hidden_feature = hidden_feature.transpose(1, 2)
        hidden_feature = self.body_linear(hidden_feature)
        hidden_feature = hidden_feature.transpose(1, 2).contiguous()
        hand_hidden_feature = hidden_feature.view(self.args.batch_size, 8, -1)
        hidden_feature = torch.argmax(hand_hidden_feature, dim=2)
        decoder_outputs_hands_iperiod = self.h_vqvae.decode(self.args.batch_size, self.width, latents=hidden_feature)
        decoder_outputs_hands = decoder_outputs_hands_iperiod
        # decoder_outputs_hands = decoder_outputs_hands_iperiod

        decoder_outputs_final = torch.zeros((in_data.shape[0], in_data.shape[1], 141)).cuda()
        decoder_outputs_final[:, :, 0:18] = decoder_outputs_body[:, :, 0:18]
        decoder_outputs_final[:, :, 18:75] = decoder_outputs_hands[:, :, 0:57]
        decoder_outputs_final[:, :, 75:84] = decoder_outputs_body[:, :, 18:27]
        decoder_outputs_final[:, :, 84:141] = decoder_outputs_hands[:, :, 57:114]

        return decoder_outputs_final, latent_body, body_hidden_feature, latent_hand, hand_hidden_feature, out_data
