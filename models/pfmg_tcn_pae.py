...
class PoseGenerator(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.in_size = self.audio_f + self.pose_dims + self.facial_f + 1 + 184

        self.tcn_pose = PfMG_TCN(args)
        model_path = args.root_path + '/data/beat_cache/beat_4english_15_141/weights/pfmg_tcn.bin'
        load_checkpoints(self.tcn_pose, model_path, 'pfmg_tcn')
        for param in self.tcn_pose.parameters():
            param.requires_grad=False
        
        self.hidden_size = 256
        self.n_layer = args.n_layer
        
        gating_indices = torch.tensor([(1126 + i) for i in range(40)])
        main_indices = torch.tensor([(0 + i) for i in range(1126)])
        dropout = 0.3
        gating_hidden = 64
        main_hidden = 1024
        experts = 5
...
class PfMG_TCN_PAE(PoseGenerator):
    def __init__(self, args):
        super().__init__(args)
        self.args = args 
    ...
    def forward(self, pre_seq, in_audio=None, in_facial=None, in_pae=None, in_text=None, in_id=None, in_emo=None):
        ...
        decoder_outputs_iperiod, in_data, speaker_feat_seq = self.tcn_pose(pre_seq, in_audio, in_facial, in_pae, in_text, in_id, in_emo)

        # if not self.training:

        #     for i in range(decoder_outputs_iperiod.shape[1]):

        #         if i == 0:
        #             hidden_data = torch.cat((decoder_outputs_iperiod[:, 0, :].unsqueeze(1), in_data[:, 0, :].unsqueeze(1), in_pae[:, 0, :].unsqueeze(1)), dim=2)
        #         else:
        #             hidden_data = torch.cat((decoder_outputs_iperiod[:, i, :].unsqueeze(1), in_data[:, i, :].unsqueeze(1), pre_pae[:, 0, :].unsqueeze(1)), dim=2)

        #         decoder_outputs_period, _ = self.body_GNN(hidden_data)
        #         decoder_outputs_iperiod[:, i, 0:18] = decoder_outputs_iperiod[:, i, 0:18] + decoder_outputs_period[:, 0, 0:18]
        #         decoder_outputs_iperiod[:, i, 75:84] = decoder_outputs_iperiod[:, i, 75:84] + decoder_outputs_period[:, 0, 18:27]
        ...
        #         decoder_outputs_iperiod[:, i, 18:75] = decoder_outputs_iperiod[:, i, 18:75] + decoder_outputs_period[:, 0, 0:57]
        #         decoder_outputs_iperiod[:, i, 84:141] = decoder_outputs_iperiod[:, i, 84:141] + decoder_outputs_period[:, 0, 57:114]

        #         pre_pae = decoder_outputs_period[:, :, 114:]
        ...
        hidden_feature, _ = self.LSTM(hidden_data[:, :, :])
        hidden_feature = hidden_feature[:, :, :self.hidden_size] + hidden_feature[:, :, self.hidden_size:]
        pre_pae = self.out(hidden_feature)

        hidden_data = torch.cat((hidden_data, pre_pae), dim=2)
        ...
        hidden_data = torch.cat((decoder_outputs_iperiod, in_data, pre_pae), dim=2)
        decoder_outputs_period, _ = self.hand_GNN(hidden_data)
        ...
        return decoder_outputs_final, pre_pae, speaker_feat_seq
