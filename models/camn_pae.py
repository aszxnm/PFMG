...
class PoseGenerator(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.LSTM_PAE = nn.LSTM(self.in_size, hidden_size=self.hidden_size, num_layers=args.n_layer, batch_first=True,
                          bidirectional=True, dropout=args.dropout_prob)

        self.out_pae = nn.Sequential(
            nn.Linear(self.hidden_size, self.hidden_size//2),
            nn.LeakyReLU(True),
            nn.Linear(self.hidden_size//2, 40)
        )

        gating_indices = torch.tensor([(446 + i) for i in range(40)])
        main_indices = torch.tensor([(0 + i) for i in range(446)])
        dropout = 0.3
        gating_hidden = 64
        main_hidden = 1024
        experts = 5
        ...
        gating_indices = torch.tensor([(473 + i) for i in range(40)])
        main_indices = torch.tensor([(0 + i) for i in range(473)])
        dropout = 0.3
        gating_hidden = 64
        main_hidden = 1024
        experts = 5
...
class CAMN_PAE(PoseGenerator):
    ...
    def forward(self, pre_seq, in_audio=None, in_facial=None, in_pae=None, in_text=None, in_id=None, in_emo=None):
        ...
        hidden_feature, _ = self.LSTM_PAE(in_data[:, :, :])
        hidden_feature = hidden_feature[:, :, :self.hidden_size] + hidden_feature[:, :, self.hidden_size:]
        pre_pae = self.out_pae(hidden_feature)

        in_data = torch.cat((in_data, pre_pae), dim=2)
        ...
        return decoder_outputs_final, pre_pae
